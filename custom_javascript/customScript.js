...
$(document).ready(function () {
    $("#clickSignUp").click(function () {
        ...
        if (!ifChecked) {
            status++;
            error('<strong>Please accept our terms & conditions.</strong>');
        }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (obj.output == "success") {
                ...
                success(obj.msg);
                var valTimeout = 60; //600 seconds = 10 minutes
                var valTimeoutForStorage = valTimeout * 1000;
                $.jStorage.set("isSet", "true"); //setting the status for isSet to true and passing a timeout parameter
                $.jStorage.set("remainSeconds", valTimeout, {TTL: valTimeoutForStorage}); //60 seconds
                var countdownTimer = setInterval(myTimer, 1000);
//                alert("Done");
            } else {
                error(obj.msg);
            }
...
//Function to keep cart reset timer

function myTimer() {
    var isSet = "";
    var seconds = 0;
    var timerText = "";
//$.jStorage.set("isSet", "false"); //setting the status for isSet to true and passing a timeout parameter
    isSet = $.jStorage.get("isSet");
//    alert(isSet);
    if (isSet == "true") {
        seconds = $.jStorage.get("remainSeconds");
//        alert("SET " + seconds);
        ...
        timerText = minutes + ":" + remainingSeconds;
...
//            document.getElementById('countdown').innerHTML = "Completed";
            ...
            seconds--;
            var timeForStorage = seconds * 1000;
            $.jStorage.set("remainSeconds", seconds, {TTL: timeForStorage}); //60 seconds
            $('.popup_box').slideDown();
            $(".popup_box .popup_box_h3").text("YOUR CART WILL EXPIRE AFTER " + timerText + " MINUTES.");
        }
    }
//    else if(isSet == null){
//        seconds = valTimeout;
//        
////        alert("NOT SET " + seconds);
//
//        var days = Math.floor(seconds / 24 / 60 / 60);
//        var hoursLeft = Math.floor((seconds) - (days * 86400));
//        var hours = Math.floor(hoursLeft / 3600);
...
function verifyAddressID() {
    ...
    var ifChecked = false;
    var ifDeliver = false;
    var valRadio = $("input[name=method]:checked").val();

//    $('input#radioPickup').on('ifChecked', function () {
//        ifChecked = true;
//    });
//
//    $('input#radioDeliver').on('ifChecked', function () {
//        ifChecked = true;
//        ifDeliver = true;
//    });
//    
//    alert();
//    alert($("input#radioPickup:checked").val());

//    alert(ifDeliver);

    if (typeof valRadio == "undefined") {
        error("<strong>Please choose one option at least.</strong>");
        $(".txtMethod").css('color', '#CC0000');
    } else {
        $(".txtMethod").css('color', '#599b09');
        if(valRadio == "deliver") {
        ...
        if (flag == 0 || typeof valRadio == "undefined") {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (obj.output === "success") {
                    ...
                    var bookedSeat = obj.BookedSeat;
                    var showSelect = '';
                    var arrBookedSeat = "";
                    var isSeatNotApplicable = obj.isSeatNotApplicable;
                    ...
                    alert(isSeatNotApplicable);
                    if (isSeatNotApplicable == "yes") {
                        seatHtml += '<h5 class="text-center">Seat Plan not available for this gallery.</h5><br/>';
                        seatHtml += '<div class="text-center"><button type="button" class="btn btn-primary" onclick="javascript:selectSeat(' + coordinateId + ',' + SeatCount + ',' + placeId + ',' + eventId + ',' + venueId + ',' + obj.ticketPrice + ',\'No\')"><i class="fa fa-cart-plus"></i> Buy Tickets</button></div>';
                    } else {
                        if (column > 0 && row > 0) {
                            for (var a = 0; a < row; a++) {
                                ...
                                for (var b = 0; b < column; b++) {
                                    if (objDefaultSeatPlan.hasOwnProperty(count)) {
                                        if (objEventSeatPlan.hasOwnProperty(count)) {
                                            ...
                                            arrBookedSeat = bookedSeat.split(',');
                                            if ($.inArray(SeatCount.toString(), arrBookedSeat) >= 0) {
                                                ...
                                                if (objSelectedSeat.length > 0) {
                                                    if (objSelectedSeat.indexOf(customSeatId) >= 0) {
                                                        ...
                                                        seatHtml += '<li><a id="seatId_' + coordinateId + '_' + SeatCount + '" onclick="javascript:selectSeat(' + coordinateId + ',' + SeatCount + ',' + placeId + ',' + eventId + ',' + venueId + ',' + obj.ticketPrice + ',\'Yes\')" data-toggle="tooltip" data-placement="top" title="Price TK.' + obj.ticketPrice + '"  class="seatTooltip available"><span>' + SeatCount + '</span></a> </li>';
                                                        SeatCount++;
                                                    }
                                                } else {
                                                    seatHtml += '<li><a id="seatId_' + coordinateId + '_' + SeatCount + '" onclick="javascript:selectSeat(' + coordinateId + ',' + SeatCount + ',' + placeId + ',' + eventId + ',' + venueId + ',' + obj.ticketPrice + ',\'Yes\')" data-toggle="tooltip" data-placement="top" title="Price TK.' + obj.ticketPrice + '"  class="seatTooltip available"><span>' + SeatCount + '</span></a> </li>';
...
//function to add specific seat to cart. using type we can also add tickets that has no specific seat no
function selectSeat(coordinateId, seatId, placeId, eventId, venueId, ticktPrice, isSeatNo) {
    alert("clicked");
    var seatSelectedId = $("#seatSelected").val();
...
$(document).ready(function () {
    ...
    $('input#MI_is_closed_event_check').on('ifChecked', function () {
        $('input#MI_is_closed_event').val("yes");
    });

    $('input#MI_is_closed_event_check').on('ifUnchecked', function () {
        $('input#MI_is_closed_event').val("no");
    });
});


// Save and send email to event request data
$(document).ready(function () {
    $("#btnSaveEventRequest").click(function () {
        var MI_first_name = $("#MI_first_name").val();
        var MI_last_name = $("#MI_last_name").val();
        var MI_email_address = $("#MI_email_address").val();
        var MI_address = $("#MI_address").val();
        var MI_number = $("#MI_number").val();
        var MI_event_title = $("#MI_event_title").val();
        var MI_event_date_time = $("#MI_event_date_time").val();
        var MI_is_closed_event = "no";
        var MI_event_description = $("#MI_event_description").val();
        var MI_about_ticket = $("#MI_about_ticket").val();
        var MI_venue_name = $("#MI_venue_name").val();
        var MI_venue_address = $("#MI_venue_address").val();
        $('input#MI_is_closed_event').on('ifChecked', function () {
            MI_is_closed_event = "yes";
        });
        ...
        var flag = 0;
        if (MI_first_name === "") {
            error('<strong>Enter first name</strong>');
            $("#MI_first_name").addClass("inputError");
            flag++;
        } else {
            $("#MI_last_name").removeClass("inputError");
        }
        if (MI_last_name === "") {
            error('<strong>Enter last name</strong>');
            $("#MI_last_name").addClass("inputError");
            flag++;
        } else {
            $("#MI_last_name").removeClass("inputError");
        }
        if (MI_email_address === "") {
            error('<strong>Enter email address</strong>');
            $("#MI_email_address").addClass("inputError");
            flag++;
        } else {
            $("#MI_email_address").removeClass("inputError");
        }
        if (MI_email_address != "" && !validateEmail(MI_email_address)) {
            $("#MI_email_address").addClass("inputError");
            flag++;
            error('<strong>Invalid email given</strong>');
        }
        if (MI_address === "") {
            error('<strong>Enter address</strong>');
            $("#MI_address").addClass("inputError");
            flag++;
        } else {
            $("#MI_address").removeClass("inputError");
        }
        if (MI_number === "") {
            error('<strong>Enter number</strong>');
            $("#MI_number").addClass("inputError");
            flag++;
        } else {
            $("#MI_number").removeClass("inputError");
        }
        if (MI_event_title === "") {
            error('<strong>Enter event title</strong>');
            $("#MI_event_title").addClass("inputError");
            flag++;
        } else {
            $("#MI_event_title").removeClass("inputError");
        }
        if (MI_event_date_time === "") {
            error('<strong>Enter event date and time</strong>');
            $("#MI_event_date_time").addClass("inputError");
            flag++;
        } else {
            $("#MI_event_date_time").removeClass("inputError");
        }
        if (MI_about_ticket === "") {
            error('<strong>Enter ticket description</strong>');
            $("#MI_about_ticket").addClass("inputError");
            flag++;
        } else {
            $("#MI_about_ticket").removeClass("inputError");
        }
        if (MI_venue_name === "") {
            error('<strong>Enter veneu name</strong>');
            $("#MI_venue_name").addClass("inputError");
            flag++;
        } else {
            $("#MI_venue_name").removeClass("inputError");
        }
        if (MI_venue_address === "") {
            error('<strong>Enter venue address</strong>');
            $("#MI_venue_address").addClass("inputError");
            flag++;
        } else {
            $("#MI_venue_address").removeClass("inputError");
        }

        if (flag == 0) {
            $.ajax({
                type: "POST",
                url: baseUrl + "ajax/ajaxSaveMerchantData.php",
                dataType: "json",
                data: {
                    MI_first_name: MI_first_name,
                    MI_last_name: MI_last_name,
                    MI_email_address: MI_email_address,
                    MI_address: MI_address,
                    MI_number: MI_number,
                    MI_event_title: MI_event_title,
                    MI_event_date_time: MI_event_date_time,
                    MI_is_closed_event: MI_is_closed_event,
                    MI_event_description: MI_event_description,
                    MI_about_ticket: MI_about_ticket,
                    MI_venue_name: MI_venue_name,
                    MI_venue_address: MI_venue_address
                },
                success: function (response) {
                    var obj = response;
                    if (obj.output == "success") {
...
$(document).ready(function () {
    $("#btnSaveContactUs").click(function () {
        ...
        var flag = 0;
        if (CU_name === "") {
            error('<strong>Enter name</strong>');
            $("#CU_name").addClass("inputError");
            flag++;
        } else {
            $("#CU_name").removeClass("inputError");
        }
        if (CU_email === "") {
            error('<strong>Enter email</strong>');
            $("#CU_email").addClass("inputError");
            flag++;
        } else {
            $("#CU_email").removeClass("inputError");
        }
        if (CU_email != "" && !validateEmail(CU_email)) {
            $("#CU_email").addClass("inputError");
            flag++;
            error('<strong>Invalid email given</strong>');
        }
        if (CU_subject === "") {
            error('<strong>Enter subject</strong>');
            $("#CU_subject").addClass("inputError");
            flag++;
        } else {
            $("#CU_subject").removeClass("inputError");
        }
        if (CU_message === "") {
            error('<strong>Enter message</strong>');
            $("#CU_message").addClass("inputError");
            flag++;
        } else {
            $("#CU_message").removeClass("inputError");
        }
        ...
        if (flag == 0) {
            ...
            $.ajax({
                type: "POST",
                url: baseUrl + "ajax/ajaxSaveContactUsData.php",
                dataType: "json",
                data: {
                    CU_name: CU_name,
                    CU_email: CU_email,
...
$(document).ready(function () {
    ...
    $('input#radioPickup').on('ifChecked', function () {
        $('#pickupAddress').fadeIn();
        $('#shippingAddress').fadeOut();
        $('#checkBillToShip').fadeOut();
        initialize();
    });

    $('input#radioDeliver').on('ifChecked', function () {
        $('#pickupAddress').fadeOut();
        $('#shippingAddress').fadeIn();
        $('#checkBillToShip').fadeIn();
    });
});